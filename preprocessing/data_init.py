...
from preprocessing.datasets.dataset import get_splits, DataPoint, DatasetBasis
...
def init_data(args:dict, seed=1, tmp_bool_cutouts:bool=False):
    if args["problem"] == "allin1":

        # NO CUTOUTS
        if not tmp_bool_cutouts: 
            dataset = DatasetBasis(args["data_prep"])
