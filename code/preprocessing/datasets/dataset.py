...
class DataPoint(DatasetBasis):
    def __init__(self, path:str, i:int=0):
        ...
        if isinstance(i, int):
            ...
            self.input_names = [f"Sim_{get_run_ids_from_prep(self.path / "Inputs")[ii]}.pt" for ii in i]
            self.label_names = [f"Sim_{get_run_ids_from_prep(self.path / "Labels")[ii]}.pt" for ii in i]
        else:
